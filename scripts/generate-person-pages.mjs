...
<link rel="icon" href="/favicon.png"><link rel="stylesheet" href="/style.css"><script type="application/ld+json">${jsonLd}</script><script src="/bt-analytics-gate.js"></script>
...
</main><script>window.va=window.va||function(){(window.vaq=window.vaq||[]).push(arguments)};const trackEvent=(name,data)=>window.va("event",{name,data:{service_id:"sage-scroll",locale:"${lang}",...data}});trackEvent("person_open",{content_id:"${escapeHtml(person.id)}",placement:"person-profile",source_type:"internal"});${(person.works || []).length ? `trackEvent("works_view",{content_id:"${escapeHtml(person.id)}",placement:"person-profile",source_type:"editorial"});` : ""}document.addEventListener("click",event=>{const link=event.target.closest("a.book-shop[data-content-id]");if(link)trackEvent("affiliate_click",{network:link.dataset.network||"amazon",content_id:(link.dataset.contentId||"unknown").slice(0,80),placement:link.dataset.placement||"person-profile-work",source_type:"affiliate"})});document.getElementById("share-profile").addEventListener("click",async()=>{const u=new URL("${path}",location.origin);u.searchParams.set("utm_source","sage-scroll");u.searchParams.set("utm_medium","share");u.searchParams.set("utm_campaign","person_profile");try{if(navigator.share){await navigator.share({title:${JSON.stringify(name)},url:u.toString()})}else{await navigator.clipboard.writeText(u.toString());alert(${JSON.stringify(en ? "Share link copied." : "공유 링크를 복사했습니다.")})}}catch(e){}});</script></body></html>`;
}
...
const entries = [
  ...
  { ko: `${baseUrl}/methodology`, en: `${baseUrl}/en/methodology`, xDefault: false },
  { ko: `${baseUrl}/media`, en: `${baseUrl}/en/media`, xDefault: false },
  ...people.map((person) => ({ ko: `${baseUrl}/person/${person.id}`, en: `${baseUrl}/en/person/${person.id}` })),
];
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml">\n${entries.flatMap(({ ko, en, xDefault = true }) => [ko, en].map((loc) => `  <url><loc>${loc}</loc><xhtml:link rel="alternate" hreflang="ko-KR" href="${ko}"/><xhtml:link rel="alternate" hreflang="en" href="${en}"/>${xDefault ? `<xhtml:link rel="alternate" hreflang="x-default" href="${ko}"/>` : ""}</url>`)).join("\n")}\n</urlset>\n`;
