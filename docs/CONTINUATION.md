...
- 마커 22곳 / 검증 19/19 / 생애지도(연표 📍 클릭→지도 이동) 작동. (2026-06-30 좌표 +3)
...
## 다음 할 일
1. **좌표 보완** — (2026-06-30 일부 완료)
   - ✅ OSM 지오코딩 성공 3곳: 정지용 문학관(옥천)·세종 영릉(여주)·이충무공 묘(아산). → 정지용 첫 마커 확보.
   - ❌ OSM 미등록(이름 태그 없음): 한용운 생가지·심우장·신채호 생가지·신채호 사당묘·김환기 고택. **Nominatim/Overpass/Photon 모두 실패.** → 카카오/네이버 지오코딩 키 또는 국가유산청(문화재청) GIS 좌표 필요. **AI가 기억으로 수치 박는 것 금지(가드레일).**
   - 최한기: `places:[]` (개성 출생=추정·북한). 검증 연고지 없음 → 마커 없음이 정상.
